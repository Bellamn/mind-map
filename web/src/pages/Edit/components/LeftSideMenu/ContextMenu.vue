<template>
  ...
  >
    <template v-if="type === 'chapter' || type === 'note'">
      <div
        class="item"
        @click.stop.prevent="(e)=>{exec('RENAME')}"
      >
        重命名
      </div>
      <div
      ...
      <div class="item danger" @click="exec('REMOVE')">删除节点</div>
      <div class="item" @click="exec('COPY')">复制节点</div>
      <div class="item" @click="exec('CUT')">剪切节点</div>
      <div
        class="item"
        :class="{ disabled: copyKey !== type }"
        @click="exec('PASTE', copyKey !== type)"
      >
        粘贴节点
      </div>
    </template>
    <template v-if="type === 'book'">
      <div class="item" @click="exec('RETURN_CENTER')">同步此笔记本</div>
      <div class="item" @click.stop.prevent="(e)=>{exec('RENAME')}">重命名</div>
      <div class="item" @click="exec('UNEXPAND_ALL')">笔记本颜色</div>
    </template>
  </div>
</template>
...
export default {
  ...
  created() {
    this.$bus.$on("menu_contextmenu", this.show);
  ...
  beforeDestroy() {
    this.$bus.$off("menu_contextmenu", this.show);
    ...
    exec(key, disabled) {
      ...
      switch (key) {
        ...
        case "CUT_NODE":
          ...
          break;
        case "PASTE":
          this.$bus.$emit("leftMenu_paste", this.type, this.ind);
          break;
        case "CREATE":
          this.$bus.$emit("leftMenu_create", this.type, this.ind);
          break;
        case "RENAME":
          this.$bus.$emit("leftMenu_rename", this.type, this.ind);
          break;
        case "REMOVE":
          this.$bus.$emit("leftMenu_remove", this.type, this.ind);
          break;
        case "COPY":
          this.copyKey = this.type
          this.$bus.$emit("leftMenu_copy", this.type, this.ind);
          break;
        case "CUT":
          this.copyKey = this.type
          this.$bus.$emit("leftMenu_cut", this.type, this.ind);
          break;
        default:
